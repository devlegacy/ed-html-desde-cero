...
exports.defaultJoin = createJoinForPredicate(
  function predicate(_, uri, base, i, next) {
    ...
    if (path.resolve(absolute).includes('node_modules\\nib\\lib')) {
      console.log(path.resolve(absolute).replace('node_modules\\nib\\lib', 'src\\assets'));
      return absolute = path.resolve(absolute).replace('node_modules\\nib\\lib', 'src\\assets');
    } else if (path.resolve(absolute).includes('\\styl\\img\\dist\\mapplic\\')) {
      return absolute = path.resolve(absolute).replace('\\styl', '');
    } else if (path.resolve(absolute).includes('\\styl\\img\\dist\\logos\\')) {
      return absolute = path.resolve(absolute).replace('\\styl', '');
    } else if (path.resolve(absolute).includes('\\styl\\modules\\fonts\\')) {
      return absolute = path.resolve(absolute).replace('\\styl\\modules', '');
    }
